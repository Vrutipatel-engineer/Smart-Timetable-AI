...
st.set_page_config(page_title="AI Scheduler", layout="wide")
st.title("📅 AI Calendar Management System")
st.markdown("---")

selected = option_menu(
    menu_title=None,
    options=["Dashboard", "AI Scheduler", "Create Event","Upcoming Events","Semester / Term Templates", "Class Schedule",
             "Assignments", "Course Scheduling","Exam Planner","Holidays"],
...
# ================================================================================
# NAVBAR PAGE HANDLING
# ================================================================================
if selected == "Dashboard":
    st.title("📊 Dashboard")

    today = str(date.today())
    today_events = [e for e in st.session_state.events if e["date"] == today]
    ...
    for event in events:   # 👈 IMPORTANT LOOP
    ...
    if not events:
        ...
        if not df.empty:
            ...
            st.warning("No valid events to display")
...
# 🎓 WEEK 5-6 FEATURES
# ==================================================
elif selected == "Semester / Term Templates":
...
elif selected == "Course Scheduling":
    ...
    for e in st.session_state.events:
        calendar_events.append({
            "title": e["title"],
            "start": f"{e['date']}T{e['start']}",
            "end": f"{e['date']}T{e['end']}",
            "color": "#3788d8"
        })
